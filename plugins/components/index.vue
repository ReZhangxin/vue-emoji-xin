...
export default {
  ...
  computed: {
    panelCalss() {
      return this.iconConfig.placement === undefined
        ? 'emoji-top'
        : `emoji-${this.iconConfig.placement}`
    },
    iconSize() {
      return this.iconConfig.size === undefined ? '30px' : this.iconConfig.size
    },
    iconName() {
      return this.iconConfig.name === undefined
        ? 'fa-smile-o'
        : this.iconConfig.name
    },
    iconColor() {
      return this.iconConfig.color === undefined
        ? '#278dff'
        : this.iconConfig.color
    }
  },
  data() {
    return {
